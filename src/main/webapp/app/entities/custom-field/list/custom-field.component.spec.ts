...
import { sampleWithRequiredData } from '../custom-field.test-samples';
...
describe('CustomField Management Component', () => {
  ...
  it('Should call load all on init', () => {
    ...
    expect(comp.customFields?.[0]).toEqual(expect.objectContaining({ id: 'ABC' }));
  });

  describe('trackId', () => {
    it('Should forward to customFieldService', () => {
      const entity = { id: 'ABC' };
      jest.spyOn(service, 'getCustomFieldIdentifier');
      const id = comp.trackId(0, entity);
      expect(service.getCustomFieldIdentifier).toHaveBeenCalledWith(entity);
