...
import customFieldResolve from './custom-field-routing-resolve.service';

describe('CustomField routing resolve service', () => {
  let mockRouter: Router;
  let mockActivatedRouteSnapshot: ActivatedRouteSnapshot;
  let service: CustomFieldService;
  let resultCustomField: ICustomField | null | undefined;
  ...
  describe('resolve', () => {
    it('should return ICustomField returned by find', () => {
      ...
      TestBed.runInInjectionContext(() => {
        customFieldResolve(mockActivatedRouteSnapshot).subscribe({
          next(result) {
      ...
      expect(resultCustomField).toEqual({ id: 'ABC' });
    });
    ...
    it('should return null if id is not provided', () => {
      ...
      expect(resultCustomField).toEqual(null);
    });

    it('should route to 404 page if data not found in server', () => {
      // GIVEN
      jest.spyOn(service, 'find').mockReturnValue(of(new HttpResponse<ICustomField>({ body: null })));
      mockActivatedRouteSnapshot.params = { id: 'ABC' };
